...
test("writeModelsConfig synchronizes enabledModels and defaultProvider in settings.json", (t) => {
  const root = createTempRoot(t);
  const modelsPath = join(root, "models.json");
  ...
  // Add a new provider "opencode"
  writeModelsConfig(
    {
      providers: {
        deepseek: { models: [{ id: "deepseek-v4-pro" }] },
        opencode: { models: [{ id: "glm-5.2" }] },
      },
    },
    modelsPath,
    settingsPath,
  ...
  const afterAdd = JSON.parse(readFileSync(settingsPath, "utf8"));
  assert.deepEqual(afterAdd.enabledModels, ["deepseek/deepseek-v4-pro", "opencode/glm-5.2"]);
  assert.equal(afterAdd.defaultProvider, "deepseek");
  ...
  const afterRemove = JSON.parse(readFileSync(settingsPath, "utf8"));
  assert.deepEqual(afterRemove.enabledModels, ["opencode/glm-5.2"]);
  assert.equal(afterRemove.defaultProvider, undefined);
  assert.equal(afterRemove.defaultModel, undefined);
});

test("syncSettingsForProviderAuth adds and removes auth providers from enabledModels", (t) => {
  const root = createTempRoot(t);
  const settingsPath = join(root, "settings.json");
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      enabledModels: ["deepseek/*", "anthropic/*"],
    }, null, 2),
  );

  syncSettingsForProviderAuth("anthropic", "removed", settingsPath);
  const afterRemove = JSON.parse(readFileSync(settingsPath, "utf8"));
  assert.deepEqual(afterRemove.enabledModels, ["deepseek/*"]);
  assert.equal(afterRemove.defaultProvider, undefined);

  syncSettingsForProviderAuth("openai", "added", settingsPath);
  const afterAdd = JSON.parse(readFileSync(settingsPath, "utf8"));
  assert.deepEqual(afterAdd.enabledModels, ["deepseek/*", "openai/*"]);
});
