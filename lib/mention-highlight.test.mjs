...
test("matches full file paths containing slashes — no truncation at '/'", () => {
  const segments = parseMentions("打开 @.pi/agent/config.json 看看");
  assert.deepEqual(segments, [
    { kind: "text", text: "打开 " },
    { kind: "file", text: "@.pi/agent/config.json" },
    { kind: "text", text: " 看看" },
  ]);
  assert.equal(hasMention("@.pi/agent/config.json"), true);
});

test("keeps URLs intact — path segments are not treated as commands", () => {
  const url = "https://github.com/cathrynlavery/diagram-design";
  const segments = parseMentions("check " + url + " now");
  // No mention matches anywhere in the URL, so the whole line stays one text segment.
  assert.deepEqual(segments, [{ kind: "text", text: "check " + url + " now" }]);
  assert.equal(hasMention(url), false);
});

test("still recognizes commands after a space in the middle of text", () => {
  const segments = parseMentions("please run /compact here");
  assert.deepEqual(segments, [
