...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const [attachedFiles, setAttachedFiles] = useState<AttachedFile[]>(() => (
    draftKey ? (getDraft(draftKey)?.files ?? []).map((f) => ({ path: f.path, size: f.size })) : []
  ));
  ...
  useImperativeHandle(ref, () => ({
    ...
    rekeyDraft(previousKey: string, nextKey: string) {
      ...
      const currentDraft = {
        ...
        files: attachedFilesRef.current.map((f): ChatDraftFile => ({ path: f.path, size: f.size })),
      };
      ...
      const unchanged = moved.value === currentDraft.value
        ...
        && moved.files.every((file, index) => (
          file.path === currentDraft.files[index]?.path
          && file.size === currentDraft.files[index]?.size
        ));
      draftKeyRef.current = nextKey;
      if (unchanged) return;

      const movedImages = draftImagesToAttachedImages(moved.images);
      const movedFiles = (moved.files ?? []).map((f) => ({ path: f.path, size: f.size }));
    ...
    restoreSubmission(text: string, images?: ChatDraftImage[], targetDraftKey?: string) {
      ...
      attachedImagesRef.current = restoredImages;
      attachedFilesRef.current = restoredDraft.files.map((f) => ({ path: f.path, size: f.size }));
      setValue((current) => {
    ...
    async addFiles(files: File[]) {
      ...
      const fileUploads = files.filter((f) => !f.type.startsWith("image/"));
      if (imageFiles.length) processImageFiles(imageFiles);
      if (fileUploads.length) await addFileUploads(fileUploads);
    },
  }));
  ...
  const addFileUploads = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      setAttachedFiles((prev) => [
        ...prev,
        ...paths.map((p, i) => ({ path: p, size: files[i]?.size ?? 0 })),
      ]);
    } catch (err) {
      console.error("File upload failed:", err);
      setFileUploadError(t("chat.fileUploadFailed"));
      setTimeout(() => setFileUploadError(null), 4000);
    }
  }, [isStreaming, t]);
  ...
  useEffect(() => {
    ...
    setDraft(draftKey, {
      ...
      files: attachedFiles.map((f): ChatDraftFile => ({ path: f.path, size: f.size })),
    });
  }, [attachedImages, attachedFiles, draftKey, value]);
  ...
  useEffect(() => {
    ...
    const nextFiles = (draft?.files ?? []).map((f) => ({ path: f.path, size: f.size }));
  ...
  const sendQueued = useCallback((mode: "steer" | "followup") => {
    ...
    if (msg.startsWith("/") && onPromptWithStreamingBehavior) {
      clearInput();
      onPromptWithStreamingBehavior(msg, streamingBehavior, attachedImages.length ? attachedImages : undefined, undefined);
      return;
    }
    clearInput();
    if (mode === "steer" && onSteer) {
      onSteer(msg, attachedImages.length ? attachedImages : undefined, undefined);
    } else if (mode === "followup" && onFollowUp) {
      onFollowUp(msg, attachedImages.length ? attachedImages : undefined, undefined);
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
            {attachedFiles.map((file, i) => (
              ...
              >
                ...
                <span style={{ color: "var(--text-dim)", flexShrink: 0 }}>{formatFileSize(file.size)}</span>
                <button
        ...
        {fileUploadError && (
          <div style={{ color: "var(--danger, #ef4444)", fontSize: 12, marginBottom: 6 }}>
            {fileUploadError}
          </div>
        )}
