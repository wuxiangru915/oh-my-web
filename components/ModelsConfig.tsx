...
export function ModelsConfig({
  ...
  const addDiscoveredModels = useCallback((providerName: string, discovered: DiscoveredModel[], baseUrl?: string) => {
    setConfig((prev) => {
      const provider = prev.providers?.[providerName] ?? {};
      const models = [...(provider.models ?? [])];
    ...
    autoFillImportedModels(providerName, baseUrl, discovered.map((model) => model.id));
  }, [autoFillImportedModels]);
    ...
    if (selection.type === "provider") {
      ...
      return (
        <ProviderDetail
          ...
          onAddModels={(models) => addDiscoveredModels(selection.name, models, provider.baseUrl)}
        />
      );
    }
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: isMobile ? "column" : "row", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <div style={{ borderTop: "1px solid var(--border)", padding: "8px 6px" }}>
              <button onClick={addCustomProvider} style={{
